...
export default class TextEditorState {
    ...
    addOrUpdateWatch(value, info) {
        ...
        if (!this.markersByLine.has(end.line)) {
            ...
            this.markersByLine.set(end.line, { id, marker, overlay, element });
        }
    ...
    reset() {
        ...
        this.selections.forEach(selection => selection.destroy());
    }

    getSelectionForPoint(point) {
        for (const selection of this.selections) {
            const range = selection.marker.getBufferRange();
    ...
    addWatchSelection() {
        const watcher = new WatcherSelection(
            this.textEditor, this.sendMessage);
        this.selections.push(watcher);
    }

    widenCurrentSelection() {
        const position = this.textEditor.getCursorBufferPosition();
        const selection = this.getSelectionForPoint(position);
        if (selection) {
            selection.expandRange();
        }
    }

    narrowCurrentSelection() {
        const position = this.textEditor.getCursorBufferPosition();
        const selection = this.getSelectionForPoint(position);
        if (selection) {
            selection.narrowRange();
            this.removeInspectorsNotInSelection();
        }
    }

    removeInspectorsNotInSelection() {
        console.log('TODO: Remove inspectors no longer contained in a selection')
    }
